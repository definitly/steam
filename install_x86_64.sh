  ...
  if ! [ -z "$(dmesg | grep radeon)" ] ; then 
        ...
        echo 'RADEON'
              ...
              if ! [ -f "tar/linux-c6-dri-11.0.7.txz" ]; then 
                ...
                if ! [ -f "tar/mesa-private-llvm-3.6.2-1.el6.x86_64.rpm" ]; then 

                     cd tar && fetch ftp://195.220.108.108/linux/centos/6.8/os/x86_64/Packages/mesa-private-llvm-3.6.2-1.el6.x86_64.rpm
                     cd ..
                  fi

              cd tar &&  rpm2cpio.pl   mesa-private-llvm-3.6.2-1.el6.x86_64.rpm | cpio -idmv 
          cd ..
          cp tar/usr/lib64/libLLVM-3.6-mesa.so               ubuntu/usr/lib
